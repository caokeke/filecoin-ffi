    ...
    T: AsRef<Path>,
{
    ...
    let (config, comm_d) = measure_op(Operation::CommD, || -> Result<_> {
        ...
        let mkpath: PathBuf = ["/kjp1"].iter().collect();
        let path = "/kjp1/merkle.json";
        let com_d:[u8;32];
        match std::fs::read_to_string(path) {
            Ok(file) => {
                let v: Melker = serde_json::from_str(&file).unwrap();
                config.size = v.size;
                com_d = v.comm_d;
                let to_path = StoreConfig::data_path(&config.path, &config.id);
                let from_path = StoreConfig::data_path(&mkpath, &config.id);
                Command::new("ln").arg("-s").arg(from_path).arg(to_path).output().expect("执行异常，提示");
            ...
            Err(_e) => {
                ...
                com_d = commitment_from_fr(comm_d_root);
                drop(data_tree);
                let from_path = StoreConfig::data_path(&config.path, &config.id);
                let to_path = StoreConfig::data_path(&mkpath, &config.id);
                fs::copy(&from_path, &to_path).with_context(|| {
                    format!(
                        "could not copy in_path={:?} to out_path={:?}",
                        from_path,
                ...
                let j = serde_json::to_string(&mk);
                let mut file = std::fs::File::create(path).expect("create failed");
                file.write_all(j.unwrap().as_bytes()).expect("write failed");
